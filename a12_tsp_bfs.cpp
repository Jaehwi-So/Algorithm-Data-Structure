...
int get_bound(int i, int start) {
	...
	for (int j = 0; j < a_num; j++) {
		for (int k = 0; k < a_num; k++) {
			if (min_v > adj_mat[a[j]][a[k]] && a[j] != a[k]) {
				min_v = adj_mat[a[j]][a[k]];
			}
		}
		if (min_v > adj_mat[a[j]][start] && a[j] != start) {
			min_v = adj_mat[a[j]][start];
		}
		sum_v += min_v;
		min_v = INT_MAX;
	}
...
Node tsp_func(int i, int start) {
	...
	Node u = create_node();	//큐에 삽입할 마디(가지뻗기 시도할 마디)
	Node v = create_node();	//큐에서 꺼낼 마디(탐색된 마디)
	Node r = create_node();	//최적해 마디
	u.i = 0;	//수준 초기화
	path[0] = start;	//시작 정점 초기화
	u.bound = get_bound(i, start);	//기댓값 계산
	for (int k = 0; k <= i; k++) {	//큐에 넣기 전에 경로 정보 복사
		u.pt[k] = path[k];
	}
	r = u;	//최선의 경우 저장
	init_queue(q);
	...
	//너비우선 탐색
	while (is_empty(q) != 1) {
		v = dequeue(q);	//큐에서 다음 마디를 꺼내 탐색
		print_status(v, 0);
		for (int k = 0; k <= v.i; k++) {	//현재 경로에 큐에서 꺼낸 정보를 복사
			path[k] = v.pt[k];
		}
