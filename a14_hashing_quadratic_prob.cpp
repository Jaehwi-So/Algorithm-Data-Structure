...
//이차 조사법을 통해 키 삽입 해시 함수를 통해 얻어낸 해시값이 충돌이 일어나면 
//(해시값 + inc^2)에 moduler 연산을 한 버킷을 다음으로 탐색한다.
void quadratic_probing_insert(element item, element table[]) {
...
void quadratic_probing_search(element item, element table[]) {
	int hash_value = hash_function(item.key);
	int i = hash_value;
	int inc = 0;
	while (!empty(table[i])) {	//빈 버킷을 찾을 때 까지
...
int main(void) {
	...
	for (int i = 0; i < 7; i++) {
		strcpy(e.key, s[i]);
		quadratic_probing_insert(e, hash_table);
	}
	for (int i = 0; i < 7; i++) {
		strcpy(e.key, s[i]);
		quadratic_probing_search(e, hash_table);
	}
	return 0;
}